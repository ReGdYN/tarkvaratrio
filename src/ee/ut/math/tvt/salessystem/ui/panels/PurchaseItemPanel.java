...
public class PurchaseItemPanel extends JPanel {
	...
	private JComboBox nameFieldList;
	...
	private JComponent drawDialogPane() {
		...
		for (int i = 0 ; i < (model.getWarehouseTableModel().getRowCount()) ; i++) {
			nameFieldList.addItem((String) model.getWarehouseTableModel().getValueAt(i, 1));
		}
		
		nameFieldList.addActionListener(new ActionListener() {

			@Override
			public void actionPerformed(ActionEvent e) {
				JComboBox item = (JComboBox) e.getSource();
				String itemName = item.getSelectedItem().toString();
				Long itemID = model.getWarehouseTableModel().getItemIdByName(itemName);
				barCodeField.setText(itemID.toString());
				fillDialogFields();
			}
		});
	...
	public void addItemEventHandler() {
		...
		if (stockItem != null) {
			...
			else{JOptionPane.showMessageDialog(this,"Not enough items in warehouse." +
					" There are currently "+stockItem.getQuantity()+"" +
							" items in warehouse","Quantity error",JOptionPane.ERROR_MESSAGE);;}
		}
	}
	...
	public void setEnabled(boolean enabled) {
		this.nameFieldList.setEnabled(enabled);
